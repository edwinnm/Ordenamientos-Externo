...
public class NaturalFechas {
    ...
    public void proceso(String nombreArchivo) throws Exception {
        ...
        File aux1 = new File("Natural/Fechas/aux_1.csv");
        File aux2 = new File("Natural/Fechas/aux_2.cvs");
    ...
    private boolean division(File F, File F1, File F2) throws IOException, ParseException {
        String actual = null;
        ...
        while (archivo.readRecord()) {
        	...
        	//Obtengo la cuarta columna
            actual = archivo.get(3);
            
            if (anterior == null) {
                anterior = actual;
            }
            //Condicion de evaluacion de fechas 
            if (formatoFecha.parse(anterior).compareTo(formatoFecha.parse(actual)) > 0) {
    ...
    private void fusion(File F, File F1, File F2) throws IOException, ParseException {
        ...
        while (contAux1 > 0 && contAux2 > 0) {
            ...
            while (formatoFecha.parse(anterior[0]).compareTo(formatoFecha.parse(actual[0])) <= 0 && formatoFecha.parse(anterior[1]).compareTo(formatoFecha.parse(actual[1])) <= 0) {
                //indexArchivo = (actual[0].compareTo(actual[1]) <= 0) ? 0 : 1;
                indexArchivo = (formatoFecha.parse(actual[0]).compareTo(formatoFecha.parse(actual[1])) <= 0) ? 0 : 1;
            ...
            if(indexArchivo == 0 ){
                ...
                while (formatoFecha.parse(anterior[1]).compareTo(formatoFecha.parse(actual[1])) <= 0) {
            ...
            if(indexArchivo == 1){
                ...
                while (formatoFecha.parse(anterior[0]).compareTo(formatoFecha.parse(actual[0])) <= 0) {
