...
public class DirectaFechas {
	private String [] arreglo = new String[4];
	private SimpleDateFormat formatoFecha = new SimpleDateFormat("dd/MM/yyyy");
	
	public void proceso(String nombreArchivo) throws IOException, ParseException{
		
		File file = new File(nombreArchivo);
		File aux1 = new File("Directa/Booleanos/aux_1.csv");
		File aux2 = new File("Directa/Booleanos/aux_2.csv");
	...
	private void mezclaDirecta(File file, File aux1, File aux2, int n) throws IOException, ParseException {
		int part = 1;
	...
	private void fusiona(File file, File aux1, File aux2, int part) throws IOException, ParseException {
        ...
        while((esFinf1 || (B1 == false)) && (esFinf2 || (B2 == false))){
        	...
        	while(((k < part) && (B1 == false)) && ((l < part) && (B2 == false))){
                if (formatoFecha.parse(f1.get(3)).compareTo(formatoFecha.parse(f2.get(3))) <= 0) {
                    guardar(f, f1);
                    B1 = true;
                    k++;
                    esFinf1 = f1.readRecord();
